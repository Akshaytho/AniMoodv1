...
export interface BuildCreativeWorkSeriesInput {
  baseUrl: string;
  kind: 'manga' | 'manhwa';
  slug: string;
  name: string;
...
export function buildCreativeWorkSeries(input: BuildCreativeWorkSeriesInput): JsonLd {
  const url = toEntityUrl({ kind: input.kind, slug: input.slug, baseUrl: input.baseUrl });
